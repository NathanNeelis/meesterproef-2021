...
self.addEventListener('activate', (event) => {
    ...
    event.waitUntil(
        ...
        .then((keylist) => {
            return Promise.all([
                keylist
                .filter(cacheKey => cacheKey.includes('hipper-cache') && cacheKey !== CORE_CACHE_NAME) // array of strings with caches that include fam-cache but are not the current version
